...
TEST_CASE("VoltageField eval_geometry and eval_geometry_sweep over all geometries", "[ScalarField][VoltageField][eval_geometry]")
{
    ...
    SECTION("Evaluation over Line geometry")
    {
        ...
        SECTION("Single wavelength evaluation (eval_geometry)")
        {
            ...
            CHECK_THAT(positions(0, 0).z, WithinRel(-50.0, EPSILON_ABS));
            CHECK_THAT(positions(1, 0).z, WithinRel(-25.0, EPSILON_ABS));
            CHECK_THAT(positions(2, 0).z, WithinRel(00.0, EPSILON_ABS));
            CHECK_THAT(positions(3, 0).z, WithinRel(25.0, EPSILON_ABS));
            CHECK_THAT(positions(4, 0).z, WithinRel(50.0, EPSILON_ABS));

            // First, find the maximum of the physical values and normalize for better comparison
            double const abs_max = normalize(values);
            CHECK_THAT(abs_max, WithinRel(0.00310138368013728, EPSILON_ABS));

            // Verify physical values along the curve
            CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0337295527213379, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.0238992910918213, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 0)), WithinRel(1, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.0238992910918213, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0337295527213378, EPSILON_ABS));
        ...
        SECTION("Wavelength sweep evaluation (eval_geometry_sweep)")
        {
            ...
            // Verify page 1
            {
                ...
                // First, find the maximum of the physical values and normalize for better comparison
                double const abs_max = normalize(values);
                CHECK_THAT(abs_max, WithinRel(0.003411633888352, EPSILON_ABS));

                // Verify physical values along the curve
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0764583097513073, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.171220984264941, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.171220984264941, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0764583097513073, EPSILON_ABS));
            ...
            // Verify page 2
            {
                ...
                // First, find the maximum of the physical values and normalize for better comparison
                double const abs_max = normalize(values);
                CHECK_THAT(abs_max, WithinRel(0.00418396928771996, EPSILON_ABS));

                // Verify physical values along the curve
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0727453682609543, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.0279442337747555, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.0279442337747555, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0727453682609543, EPSILON_ABS));
    ...
    SECTION("Evaluation over CircleArc geometry")
    {
        ...
        SECTION("Single wavelength evaluation (eval_geometry)")
        {
            ...
            CHECK_THAT(positions(0, 0).z, WithinRel(-70.71067811865474084, EPSILON_ABS)); // t = 0.0 -> -pi/2
            CHECK_THAT(positions(1, 0).z, WithinRel(-38.26834323650897574, EPSILON_ABS)); // t = 0.25
            CHECK_THAT(positions(2, 0).z, WithinRel(0.0, EPSILON_ABS)); // t = 0.5 -> 0 rad
            CHECK_THAT(positions(3, 0).z, WithinRel(38.26834323650897574, EPSILON_ABS)); // t = 0.75
            CHECK_THAT(positions(4, 0).z, WithinRel(70.71067811865474084, EPSILON_ABS)); // t = 1.0 -> +pi/2
            ...
            // Verify physical values along the curve
            CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.00320028288208625, EPSILON_ABS));
            CHECK_THAT(std::arg(values(0, 0)), WithinRel(1.56412519106567638, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.01116871931989499, EPSILON_ABS));
            CHECK_THAT(std::arg(values(1, 0)), WithinRel(1.4187912238163809, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 0)), WithinRel(1.0, EPSILON_ABS));
        ...
        SECTION("Wavelength sweep evaluation (eval_geometry_sweep)")
        {
            ...
            // Verify page 1
            {
                ...
                // Verify physical values along the curve
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.01858547187923242, EPSILON_ABS));
                CHECK_THAT(std::arg(values(0, 0)), WithinRel(-2.61806589941934931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.01536786526331465, EPSILON_ABS));
                CHECK_THAT(std::arg(values(1, 0)), WithinRel(0.37770760072447473, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(1.0, EPSILON_ABS));
            ...
            // Verify page 2
            {
                ...
                // Verify physical values along the curve
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.02389331153927321, EPSILON_ABS));
                CHECK_THAT(std::arg(values(0, 0)), WithinRel(-1.56131841583061837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.16726023674013196, EPSILON_ABS));
                CHECK_THAT(std::arg(values(1, 0)), WithinRel(1.55375648968733615, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(1.0, EPSILON_ABS));
    ...
    SECTION("Evaluation over Rectangle geometry")
    {
        ...
        SECTION("Single wavelength evaluation (eval_geometry)")
        {
            ...
            CHECK_THAT(positions(0, 0).x, WithinRel(100, EPSILON_ABS));
            CHECK_THAT(positions(0, 0).y, WithinRel(100, EPSILON_ABS));
            CHECK_THAT(positions(0, 0).z, WithinRel(-50, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, 0).x, WithinRel(-100, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, 0).y, WithinRel(100, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, 0).z, WithinRel(-50, EPSILON_ABS));
            CHECK_THAT(positions(0, positions.shape().cols - 1).x, WithinRel(100, EPSILON_ABS));
            CHECK_THAT(positions(0, positions.shape().cols - 1).y, WithinRel(100, EPSILON_ABS));
            CHECK_THAT(positions(0, positions.shape().cols - 1).z, WithinRel(50, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, positions.shape().cols - 1).x, WithinRel(-100, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, positions.shape().cols - 1).y, WithinRel(100, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, positions.shape().cols - 1).z, WithinRel(50, EPSILON_ABS));

            // Next, find the maximum of the physical values and normalize for better comparison
            double const abs_max = normalize(values);
            CHECK_THAT(abs_max, WithinRel(0.00170301857371513, EPSILON_ABS));

            // Verify physical values along the surface
            CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0811805333041281, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 1)), WithinRel(0.0209935151691007, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 2)), WithinRel(0.181776966584661, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 3)), WithinRel(0.959227993440513, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 4)), WithinRel(0.959227993440513, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 5)), WithinRel(0.181776966584661, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 6)), WithinRel(0.0209935151689359, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 7)), WithinRel(0.0811805333041281, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.0455986942249276, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 1)), WithinRel(0.0411457551690019, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 2)), WithinRel(0.236208994298118, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 3)), WithinRel(0.980316457457755, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 4)), WithinRel(0.980316457457755, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 5)), WithinRel(0.236208994298118, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 6)), WithinRel(0.0411457551690372, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 7)), WithinRel(0.0455986942249277, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 0)), WithinRel(0.00257584589170965, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 1)), WithinRel(0.0941592075283062, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 2)), WithinRel(0.275478907113585, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 3)), WithinRel(0.99429542171149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 4)), WithinRel(0.99429542171149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 5)), WithinRel(0.275478907113585, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 6)), WithinRel(0.0941592075283062, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 7)), WithinRel(0.00257584589170968, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.0357596041928141, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 1)), WithinRel(0.128220787975769, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 2)), WithinRel(0.294897071700718, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 3)), WithinRel(1, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 4)), WithinRel(1, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 5)), WithinRel(0.294897071700718, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 6)), WithinRel(0.128220787975793, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 7)), WithinRel(0.0357596041928141, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0589060465262149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 1)), WithinRel(0.139456471473219, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 2)), WithinRel(0.294128532445929, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 3)), WithinRel(0.997606657015968, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 4)), WithinRel(0.997606657015968, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 5)), WithinRel(0.294128532445929, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 6)), WithinRel(0.139456471473219, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 7)), WithinRel(0.0589060465262149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 0)), WithinRel(0.0665616877563288, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 1)), WithinRel(0.132742825255253, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 2)), WithinRel(0.27873146020983, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 3)), WithinRel(0.989316385385742, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 4)), WithinRel(0.989316385385742, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 5)), WithinRel(0.27873146020983, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 6)), WithinRel(0.13274282525528, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 7)), WithinRel(0.0665616877563289, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 0)), WithinRel(0.0650061963206579, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 1)), WithinRel(0.118932954431791, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 2)), WithinRel(0.258936075347669, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 3)), WithinRel(0.979344504415583, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 4)), WithinRel(0.979344504415583, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 5)), WithinRel(0.258936075347669, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 6)), WithinRel(0.118932954431791, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 7)), WithinRel(0.0650061963206579, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 0)), WithinRel(0.0619228784388384, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 1)), WithinRel(0.108899738482795, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 2)), WithinRel(0.245612300311409, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 3)), WithinRel(0.972648506328136, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 4)), WithinRel(0.972648506328136, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 5)), WithinRel(0.245612300311409, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 6)), WithinRel(0.108899738482732, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 7)), WithinRel(0.0619228784388384, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 0)), WithinRel(0.0619228784388384, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 1)), WithinRel(0.108899738482794, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 2)), WithinRel(0.245612300311409, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 3)), WithinRel(0.972648506328136, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 4)), WithinRel(0.972648506328136, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 5)), WithinRel(0.245612300311409, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 6)), WithinRel(0.108899738482732, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 7)), WithinRel(0.0619228784388384, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 0)), WithinRel(0.0650061963206579, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 1)), WithinRel(0.118932954431791, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 2)), WithinRel(0.258936075347669, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 3)), WithinRel(0.979344504415583, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 4)), WithinRel(0.979344504415583, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 5)), WithinRel(0.258936075347669, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 6)), WithinRel(0.118932954431791, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 7)), WithinRel(0.0650061963206579, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 0)), WithinRel(0.0665616877563289, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 1)), WithinRel(0.132742825255253, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 2)), WithinRel(0.27873146020983, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 3)), WithinRel(0.989316385385742, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 4)), WithinRel(0.989316385385742, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 5)), WithinRel(0.27873146020983, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 6)), WithinRel(0.13274282525528, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 7)), WithinRel(0.0665616877563289, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 0)), WithinRel(0.0589060465262149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 1)), WithinRel(0.139456471473219, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 2)), WithinRel(0.294128532445929, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 3)), WithinRel(0.997606657015968, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 4)), WithinRel(0.997606657015968, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 5)), WithinRel(0.294128532445929, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 6)), WithinRel(0.139456471473219, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 7)), WithinRel(0.058906046526215, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 0)), WithinRel(0.0357596041928872, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 1)), WithinRel(0.128220787975745, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 2)), WithinRel(0.294897071701048, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 3)), WithinRel(0.999999999999537, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 4)), WithinRel(0.999999999999537, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 5)), WithinRel(0.294897071701048, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 6)), WithinRel(0.12822078797574, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 7)), WithinRel(0.0357596041928873, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 0)), WithinRel(0.00257584589170966, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 1)), WithinRel(0.0941592075283062, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 2)), WithinRel(0.275478907113585, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 3)), WithinRel(0.99429542171149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 4)), WithinRel(0.99429542171149, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 5)), WithinRel(0.275478907113585, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 6)), WithinRel(0.0941592075283062, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 7)), WithinRel(0.00257584589170969, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 0)), WithinRel(0.0455986942249276, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 1)), WithinRel(0.0411457551690019, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 2)), WithinRel(0.236208994298118, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 3)), WithinRel(0.980316457457755, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 4)), WithinRel(0.980316457457755, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 5)), WithinRel(0.236208994298118, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 6)), WithinRel(0.0411457551690372, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 7)), WithinRel(0.0455986942249276, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 0)), WithinRel(0.0811805333041281, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 1)), WithinRel(0.0209935151691007, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 2)), WithinRel(0.181776966584661, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 3)), WithinRel(0.959227993440513, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 4)), WithinRel(0.959227993440513, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 5)), WithinRel(0.181776966584661, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 6)), WithinRel(0.0209935151689359, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 7)), WithinRel(0.0811805333041281, EPSILON_ABS));
        ...
        SECTION("Wavelength sweep evaluation (eval_geometry_sweep)")
        {
            ...
            // Verify page 1
            {
                ...
                // Next, find the maximum of the physical values and normalize for better comparison
                double const abs_max = normalize(values);
                CHECK_THAT(abs_max, WithinRel(0.00262415150477076, EPSILON_ABS));

                // Verify physical values along the surface
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0813920796290424, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 1)), WithinRel(0.159917982283952, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 2)), WithinRel(0.205458647782092, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 3)), WithinRel(0.8090469509971, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 4)), WithinRel(0.8090469509971, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 5)), WithinRel(0.205458647782091, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 6)), WithinRel(0.15991798228388, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 7)), WithinRel(0.0813920796290423, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.043570731989055, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 1)), WithinRel(0.180815780118988, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 2)), WithinRel(0.15471939047631, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 3)), WithinRel(0.848680945743085, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 4)), WithinRel(0.848680945743085, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 5)), WithinRel(0.15471939047631, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 6)), WithinRel(0.180815780118965, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 7)), WithinRel(0.0435707319890549, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(0.00253488461355195, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 1)), WithinRel(0.188632002748327, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 2)), WithinRel(0.0980501762348033, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 3)), WithinRel(0.886905104645068, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 4)), WithinRel(0.886905104645068, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 5)), WithinRel(0.0980501762348032, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 6)), WithinRel(0.188632002748327, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 7)), WithinRel(0.00253488461355193, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.0365040952134442, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 1)), WithinRel(0.182481715515508, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 2)), WithinRel(0.0392879112594042, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 3)), WithinRel(0.922020581974186, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 4)), WithinRel(0.922020581974186, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 5)), WithinRel(0.0392879112594042, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 6)), WithinRel(0.182481715515508, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 7)), WithinRel(0.0365040952134442, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0666287244737948, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 1)), WithinRel(0.16489049508561, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 2)), WithinRel(0.019185876248624, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 3)), WithinRel(0.952208610160764, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 4)), WithinRel(0.952208610160764, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 5)), WithinRel(0.019185876248624, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 6)), WithinRel(0.16489049508561, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 7)), WithinRel(0.0666287244737948, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 0)), WithinRel(0.0858695121111939, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 1)), WithinRel(0.141758512009754, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 2)), WithinRel(0.0662395726249229, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 3)), WithinRel(0.975866754178091, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 4)), WithinRel(0.975866754178091, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 5)), WithinRel(0.0662395726249229, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 6)), WithinRel(0.141758512009737, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 7)), WithinRel(0.0858695121111939, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 0)), WithinRel(0.0955417665208314, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 1)), WithinRel(0.12065295827092, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 2)), WithinRel(0.100767426207421, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 3)), WithinRel(0.991938924464441, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 4)), WithinRel(0.991938924464441, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 5)), WithinRel(0.100767426207421, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 6)), WithinRel(0.12065295827092, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 7)), WithinRel(0.0955417665208314, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 0)), WithinRel(0.0990390295480543, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 1)), WithinRel(0.108229371169274, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 2)), WithinRel(0.118767855264159, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 3)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 5)), WithinRel(0.118767855264159, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 6)), WithinRel(0.108229371169274, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 7)), WithinRel(0.0990390295480543, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 0)), WithinRel(0.0990390295480544, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 1)), WithinRel(0.108229371169274, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 2)), WithinRel(0.118767855264159, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 3)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 5)), WithinRel(0.118767855264159, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 6)), WithinRel(0.108229371169274, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 7)), WithinRel(0.0990390295480543, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 0)), WithinRel(0.0955417665208314, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 1)), WithinRel(0.12065295827092, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 2)), WithinRel(0.100767426207421, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 3)), WithinRel(0.991938924464441, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 4)), WithinRel(0.991938924464441, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 5)), WithinRel(0.100767426207421, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 6)), WithinRel(0.12065295827092, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 7)), WithinRel(0.0955417665208314, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 0)), WithinRel(0.0858695121111939, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 1)), WithinRel(0.141758512009754, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 2)), WithinRel(0.0662395726249229, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 3)), WithinRel(0.975866754178091, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 4)), WithinRel(0.975866754178091, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 5)), WithinRel(0.0662395726249229, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 6)), WithinRel(0.141758512009737, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 7)), WithinRel(0.0858695121111939, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 0)), WithinRel(0.0666287244737948, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 1)), WithinRel(0.16489049508561, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 2)), WithinRel(0.019185876248624, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 3)), WithinRel(0.952208610160764, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 4)), WithinRel(0.952208610160764, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 5)), WithinRel(0.019185876248624, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 6)), WithinRel(0.16489049508561, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 7)), WithinRel(0.0666287244737948, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 0)), WithinRel(0.0365040952133544, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 1)), WithinRel(0.182481715515543, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 2)), WithinRel(0.0392879112593979, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 3)), WithinRel(0.922020581974036, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 4)), WithinRel(0.922020581974036, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 5)), WithinRel(0.0392879112593979, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 6)), WithinRel(0.1824817155155, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 7)), WithinRel(0.0365040952133545, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 0)), WithinRel(0.00253488461355196, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 1)), WithinRel(0.188632002748327, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 2)), WithinRel(0.0980501762348032, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 3)), WithinRel(0.886905104645069, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 4)), WithinRel(0.886905104645068, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 5)), WithinRel(0.0980501762348032, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 6)), WithinRel(0.188632002748327, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 7)), WithinRel(0.00253488461355191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 0)), WithinRel(0.043570731989055, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 1)), WithinRel(0.180815780118988, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 2)), WithinRel(0.15471939047631, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 3)), WithinRel(0.848680945743085, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 4)), WithinRel(0.848680945743085, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 5)), WithinRel(0.15471939047631, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 6)), WithinRel(0.180815780118965, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 7)), WithinRel(0.0435707319890549, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 0)), WithinRel(0.0813920796290424, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 1)), WithinRel(0.159917982283952, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 2)), WithinRel(0.205458647782092, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 3)), WithinRel(0.8090469509971, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 4)), WithinRel(0.8090469509971, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 5)), WithinRel(0.205458647782091, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 6)), WithinRel(0.15991798228388, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 7)), WithinRel(0.0813920796290423, EPSILON_ABS));
            ...
            // Verify page 2
            {
                ...
                // Next, find the maximum of the physical values and normalize for better comparison
                double const abs_max = normalize(values);
                CHECK_THAT(abs_max, WithinRel(0.00360682665176666, EPSILON_ABS));

                // Verify physical values along the surface
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.136351738833879, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 1)), WithinRel(0.0156554679377567, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 2)), WithinRel(0.397490031436339, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 3)), WithinRel(0.762783961753825, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 4)), WithinRel(0.762783961753825, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 5)), WithinRel(0.397490031436339, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 6)), WithinRel(0.0156554679377761, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 7)), WithinRel(0.13635173883388, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.14857503965707, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 1)), WithinRel(0.0312673485306491, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 2)), WithinRel(0.376578411378417, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 3)), WithinRel(0.806714274219831, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 4)), WithinRel(0.806714274219831, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 5)), WithinRel(0.376578411378417, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 6)), WithinRel(0.0312673485306451, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 7)), WithinRel(0.14857503965707, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(0.15070288882525, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 1)), WithinRel(0.0765947669904878, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 2)), WithinRel(0.348634440864513, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 3)), WithinRel(0.850848809701595, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 4)), WithinRel(0.850848809701595, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 5)), WithinRel(0.348634440864513, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 6)), WithinRel(0.0765947669904879, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 7)), WithinRel(0.15070288882525, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.14295381470043, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 1)), WithinRel(0.117354855779035, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 2)), WithinRel(0.314840404215442, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 3)), WithinRel(0.893433711187075, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 4)), WithinRel(0.893433711187075, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 5)), WithinRel(0.314840404215442, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 6)), WithinRel(0.117354855779031, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 7)), WithinRel(0.14295381470043, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.127883027223684, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 1)), WithinRel(0.150260701398678, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 2)), WithinRel(0.278065746582953, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 3)), WithinRel(0.932188646845362, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 4)), WithinRel(0.932188646845362, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 5)), WithinRel(0.278065746582953, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 6)), WithinRel(0.150260701398678, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 7)), WithinRel(0.127883027223684, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 0)), WithinRel(0.110001257985308, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 1)), WithinRel(0.173533460612235, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 2)), WithinRel(0.242902564630109, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 3)), WithinRel(0.964497645940459, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 4)), WithinRel(0.964497645940459, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 5)), WithinRel(0.242902564630109, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 6)), WithinRel(0.173533460612229, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 7)), WithinRel(0.110001257985308, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 0)), WithinRel(0.094473506141054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 1)), WithinRel(0.187426546889668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 2)), WithinRel(0.214959331864622, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 3)), WithinRel(0.987787141496915, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 4)), WithinRel(0.987787141496915, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 5)), WithinRel(0.214959331864622, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 6)), WithinRel(0.187426546889668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 7)), WithinRel(0.094473506141054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 0)), WithinRel(0.0855555739512042, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 1)), WithinRel(0.193613808063871, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 2)), WithinRel(0.199423301961654, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 3)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 5)), WithinRel(0.199423301961654, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 6)), WithinRel(0.193613808063838, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 7)), WithinRel(0.0855555739512041, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 0)), WithinRel(0.0855555739512042, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 1)), WithinRel(0.193613808063871, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 2)), WithinRel(0.199423301961654, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 3)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 5)), WithinRel(0.199423301961654, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 6)), WithinRel(0.193613808063838, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 7)), WithinRel(0.0855555739512041, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 0)), WithinRel(0.094473506141054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 1)), WithinRel(0.187426546889668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 2)), WithinRel(0.214959331864622, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 3)), WithinRel(0.987787141496915, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 4)), WithinRel(0.987787141496915, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 5)), WithinRel(0.214959331864622, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 6)), WithinRel(0.187426546889668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 7)), WithinRel(0.094473506141054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 0)), WithinRel(0.110001257985308, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 1)), WithinRel(0.173533460612235, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 2)), WithinRel(0.242902564630109, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 3)), WithinRel(0.964497645940459, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 4)), WithinRel(0.964497645940459, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 5)), WithinRel(0.242902564630109, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 6)), WithinRel(0.173533460612229, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 7)), WithinRel(0.110001257985308, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 0)), WithinRel(0.127883027223684, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 1)), WithinRel(0.150260701398678, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 2)), WithinRel(0.278065746582953, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 3)), WithinRel(0.932188646845362, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 4)), WithinRel(0.932188646845362, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 5)), WithinRel(0.278065746582953, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 6)), WithinRel(0.150260701398678, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 7)), WithinRel(0.127883027223684, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 0)), WithinRel(0.142953814700409, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 1)), WithinRel(0.117354855779051, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 2)), WithinRel(0.314840404215458, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 3)), WithinRel(0.893433711186985, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 4)), WithinRel(0.893433711186985, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 5)), WithinRel(0.314840404215458, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 6)), WithinRel(0.117354855779004, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 7)), WithinRel(0.142953814700408, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 0)), WithinRel(0.15070288882525, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 1)), WithinRel(0.0765947669904878, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 2)), WithinRel(0.348634440864513, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 3)), WithinRel(0.850848809701595, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 4)), WithinRel(0.850848809701595, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 5)), WithinRel(0.348634440864513, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 6)), WithinRel(0.0765947669904879, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 7)), WithinRel(0.15070288882525, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 0)), WithinRel(0.14857503965707, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 1)), WithinRel(0.0312673485306491, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 2)), WithinRel(0.376578411378417, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 3)), WithinRel(0.806714274219831, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 4)), WithinRel(0.806714274219831, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 5)), WithinRel(0.376578411378417, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 6)), WithinRel(0.0312673485306451, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 7)), WithinRel(0.14857503965707, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 0)), WithinRel(0.136351738833879, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 1)), WithinRel(0.0156554679377567, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 2)), WithinRel(0.397490031436339, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 3)), WithinRel(0.762783961753825, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 4)), WithinRel(0.762783961753825, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 5)), WithinRel(0.397490031436339, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 6)), WithinRel(0.0156554679377761, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 7)), WithinRel(0.13635173883388, EPSILON_ABS));
    ...
    SECTION("Evaluation over SphericalRectangle geometry")
    {
        ...
        SECTION("Single wavelength evaluation (eval_geometry)")
        {
            ...
            // Verify grid corner positions
            CHECK_THAT(positions(0, 0).x, WithinRel(65.32814824381881635, EPSILON_ABS));
            CHECK_THAT(positions(0, 0).y, WithinRel(65.32814824381883057, EPSILON_ABS));
            CHECK_THAT(positions(0, 0).z, WithinRel(-38.26834323650897574, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, 0).x, WithinRel(-65.32814824381881635, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, 0).y, WithinRel(65.32814824381883057, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, 0).z, WithinRel(-38.26834323650897574, EPSILON_ABS));
            CHECK_THAT(positions(0, positions.shape().cols - 1).x, WithinRel(65.32814824381881635, EPSILON_ABS));
            CHECK_THAT(positions(0, positions.shape().cols - 1).y, WithinRel(65.32814824381883057, EPSILON_ABS));
            CHECK_THAT(positions(0, positions.shape().cols - 1).z, WithinRel(38.26834323650897574, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, positions.shape().cols - 1).x, WithinRel(-65.32814824381881635, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, positions.shape().cols - 1).y, WithinRel(65.32814824381883057, EPSILON_ABS));
            CHECK_THAT(positions(positions.shape().rows - 1, positions.shape().cols - 1).z, WithinRel(38.26834323650897574, EPSILON_ABS));

            // Next, find the maximum of the physical values and normalize for better comparison
            double const abs_max = normalize(values);
            CHECK_THAT(abs_max, WithinRel(0.00214713792372675, EPSILON_ABS));

            // Verify physical values along the surface
            CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0161323981305048, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 1)), WithinRel(0.0966088241309794, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 2)), WithinRel(0.270909628866502, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 3)), WithinRel(0.999999999999391, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 4)), WithinRel(0.999999999999391, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 5)), WithinRel(0.270909628866501, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 6)), WithinRel(0.0966088241310958, EPSILON_ABS));
            CHECK_THAT(std::abs(values(0, 7)), WithinRel(0.0161323981305049, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.0161323981305376, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 1)), WithinRel(0.0966088241308526, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 2)), WithinRel(0.270909628866573, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 3)), WithinRel(0.999999999999534, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 4)), WithinRel(0.999999999999555, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 5)), WithinRel(0.270909628866573, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 6)), WithinRel(0.0966088241308649, EPSILON_ABS));
            CHECK_THAT(std::abs(values(1, 7)), WithinRel(0.0161323981305376, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 0)), WithinRel(0.0161323981306128, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 1)), WithinRel(0.0966088241309214, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 2)), WithinRel(0.270909628866805, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 3)), WithinRel(0.999999999999486, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 4)), WithinRel(0.999999999999487, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 5)), WithinRel(0.270909628866805, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 6)), WithinRel(0.0966088241309214, EPSILON_ABS));
            CHECK_THAT(std::abs(values(2, 7)), WithinRel(0.0161323981306127, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.0161323981306627, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 1)), WithinRel(0.0966088241310159, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 2)), WithinRel(0.27090962886659, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 3)), WithinRel(0.999999999999756, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 4)), WithinRel(0.999999999999756, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 5)), WithinRel(0.27090962886659, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 6)), WithinRel(0.096608824131016, EPSILON_ABS));
            CHECK_THAT(std::abs(values(3, 7)), WithinRel(0.0161323981306626, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0161323981305155, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 1)), WithinRel(0.0966088241308565, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 2)), WithinRel(0.270909628866603, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 3)), WithinRel(0.999999999999401, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 4)), WithinRel(0.999999999999401, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 5)), WithinRel(0.270909628866603, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 6)), WithinRel(0.0966088241308739, EPSILON_ABS));
            CHECK_THAT(std::abs(values(4, 7)), WithinRel(0.0161323981305156, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 0)), WithinRel(0.0161323981307005, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 1)), WithinRel(0.0966088241308201, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 2)), WithinRel(0.270909628866586, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 3)), WithinRel(0.99999999999957, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 4)), WithinRel(0.99999999999957, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 5)), WithinRel(0.270909628866585, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 6)), WithinRel(0.0966088241308295, EPSILON_ABS));
            CHECK_THAT(std::abs(values(5, 7)), WithinRel(0.0161323981307005, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 0)), WithinRel(0.0161323981305657, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 1)), WithinRel(0.0966088241307718, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 2)), WithinRel(0.270909628866542, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 3)), WithinRel(0.999999999999859, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 4)), WithinRel(1, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 5)), WithinRel(0.270909628866541, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 6)), WithinRel(0.0966088241308371, EPSILON_ABS));
            CHECK_THAT(std::abs(values(6, 7)), WithinRel(0.0161323981305657, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 0)), WithinRel(0.0161323981305536, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 1)), WithinRel(0.0966088241310956, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 2)), WithinRel(0.270909628866735, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 3)), WithinRel(0.999999999999692, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 4)), WithinRel(0.999999999999692, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 5)), WithinRel(0.270909628866735, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 6)), WithinRel(0.0966088241310184, EPSILON_ABS));
            CHECK_THAT(std::abs(values(7, 7)), WithinRel(0.0161323981305537, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 0)), WithinRel(0.0161323981305536, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 1)), WithinRel(0.0966088241310956, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 2)), WithinRel(0.270909628866735, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 3)), WithinRel(0.999999999999692, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 4)), WithinRel(0.999999999999692, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 5)), WithinRel(0.270909628866735, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 6)), WithinRel(0.0966088241310184, EPSILON_ABS));
            CHECK_THAT(std::abs(values(8, 7)), WithinRel(0.0161323981305537, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 0)), WithinRel(0.0161323981305657, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 1)), WithinRel(0.0966088241307718, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 2)), WithinRel(0.270909628866542, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 3)), WithinRel(0.999999999999859, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 4)), WithinRel(1, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 5)), WithinRel(0.270909628866542, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 6)), WithinRel(0.0966088241308371, EPSILON_ABS));
            CHECK_THAT(std::abs(values(9, 7)), WithinRel(0.0161323981305657, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 0)), WithinRel(0.0161323981307004, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 1)), WithinRel(0.0966088241308201, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 2)), WithinRel(0.270909628866586, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 3)), WithinRel(0.99999999999957, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 4)), WithinRel(0.99999999999957, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 5)), WithinRel(0.270909628866585, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 6)), WithinRel(0.0966088241308295, EPSILON_ABS));
            CHECK_THAT(std::abs(values(10, 7)), WithinRel(0.0161323981307004, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 0)), WithinRel(0.0161323981306158, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 1)), WithinRel(0.0966088241310356, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 2)), WithinRel(0.270909628866517, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 3)), WithinRel(0.999999999999496, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 4)), WithinRel(0.999999999999496, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 5)), WithinRel(0.270909628866517, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 6)), WithinRel(0.0966088241309495, EPSILON_ABS));
            CHECK_THAT(std::abs(values(11, 7)), WithinRel(0.0161323981306157, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 0)), WithinRel(0.0161323981306627, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 1)), WithinRel(0.096608824131016, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 2)), WithinRel(0.27090962886659, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 3)), WithinRel(0.999999999999756, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 4)), WithinRel(0.999999999999756, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 5)), WithinRel(0.27090962886659, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 6)), WithinRel(0.096608824131016, EPSILON_ABS));
            CHECK_THAT(std::abs(values(12, 7)), WithinRel(0.0161323981306626, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 0)), WithinRel(0.0161323981306128, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 1)), WithinRel(0.0966088241309214, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 2)), WithinRel(0.270909628866805, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 3)), WithinRel(0.999999999999486, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 4)), WithinRel(0.999999999999487, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 5)), WithinRel(0.270909628866805, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 6)), WithinRel(0.0966088241309214, EPSILON_ABS));
            CHECK_THAT(std::abs(values(13, 7)), WithinRel(0.0161323981306128, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 0)), WithinRel(0.0161323981305375, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 1)), WithinRel(0.0966088241308526, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 2)), WithinRel(0.270909628866573, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 3)), WithinRel(0.999999999999534, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 4)), WithinRel(0.999999999999555, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 5)), WithinRel(0.270909628866572, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 6)), WithinRel(0.0966088241308648, EPSILON_ABS));
            CHECK_THAT(std::abs(values(14, 7)), WithinRel(0.0161323981305376, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 0)), WithinRel(0.0161323981305049, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 1)), WithinRel(0.0966088241309794, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 2)), WithinRel(0.270909628866502, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 3)), WithinRel(0.999999999999391, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 4)), WithinRel(0.999999999999391, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 5)), WithinRel(0.270909628866501, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 6)), WithinRel(0.0966088241310958, EPSILON_ABS));
            CHECK_THAT(std::abs(values(15, 7)), WithinRel(0.0161323981305048, EPSILON_ABS));
        ...
        SECTION("Wavelength sweep evaluation (eval_geometry_sweep)")
        {
            ...
            // Verify page 1
            {
                ...
                // Next, find the maximum of the physical values and normalize for better comparison
                double const abs_max = normalize(values);
                CHECK_THAT(abs_max, WithinRel(0.00291527234595958, EPSILON_ABS));

                // Verify physical values along the surface
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.0179844363415044, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 1)), WithinRel(0.219074952276568, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 2)), WithinRel(0.130694634215046, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 3)), WithinRel(0.99999999999972, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 4)), WithinRel(0.99999999999972, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 5)), WithinRel(0.130694634215046, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 6)), WithinRel(0.219074952276612, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 7)), WithinRel(0.0179844363415045, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.017984436341519, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 1)), WithinRel(0.219074952276687, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 2)), WithinRel(0.130694634214944, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 3)), WithinRel(0.999999999999826, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 4)), WithinRel(0.999999999999837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 5)), WithinRel(0.130694634214944, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 6)), WithinRel(0.21907495227674, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 7)), WithinRel(0.0179844363415191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(0.0179844363415261, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 1)), WithinRel(0.21907495227676, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 2)), WithinRel(0.130694634214872, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 3)), WithinRel(0.999999999999829, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 4)), WithinRel(0.999999999999829, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 5)), WithinRel(0.130694634214872, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 6)), WithinRel(0.21907495227676, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 7)), WithinRel(0.0179844363415262, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.0179844363415625, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 1)), WithinRel(0.21907495227653, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 2)), WithinRel(0.130694634215113, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 3)), WithinRel(0.999999999999958, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 4)), WithinRel(0.999999999999958, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 5)), WithinRel(0.130694634215112, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 6)), WithinRel(0.21907495227653, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 7)), WithinRel(0.0179844363415626, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.0179844363414722, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 1)), WithinRel(0.219074952276715, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 2)), WithinRel(0.130694634214987, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 3)), WithinRel(0.999999999999749, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 4)), WithinRel(0.999999999999749, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 5)), WithinRel(0.130694634214987, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 6)), WithinRel(0.219074952276657, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 7)), WithinRel(0.0179844363414723, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 0)), WithinRel(0.0179844363415668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 1)), WithinRel(0.219074952276781, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 2)), WithinRel(0.130694634215074, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 3)), WithinRel(0.999999999999841, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 4)), WithinRel(0.999999999999841, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 5)), WithinRel(0.130694634215074, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 6)), WithinRel(0.219074952276729, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 7)), WithinRel(0.0179844363415668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 0)), WithinRel(0.0179844363414931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 1)), WithinRel(0.219074952276746, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 2)), WithinRel(0.130694634214992, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 3)), WithinRel(0.999999999999974, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 5)), WithinRel(0.130694634214992, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 6)), WithinRel(0.219074952276694, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 7)), WithinRel(0.0179844363414932, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 0)), WithinRel(0.0179844363414607, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 1)), WithinRel(0.219074952276704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 2)), WithinRel(0.130694634214834, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 3)), WithinRel(0.999999999999837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 4)), WithinRel(0.999999999999837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 5)), WithinRel(0.130694634214834, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 6)), WithinRel(0.219074952276674, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 7)), WithinRel(0.0179844363414608, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 0)), WithinRel(0.0179844363414607, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 1)), WithinRel(0.219074952276704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 2)), WithinRel(0.130694634214834, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 3)), WithinRel(0.999999999999837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 4)), WithinRel(0.999999999999837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 5)), WithinRel(0.130694634214834, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 6)), WithinRel(0.219074952276674, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 7)), WithinRel(0.0179844363414608, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 0)), WithinRel(0.0179844363414931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 1)), WithinRel(0.219074952276746, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 2)), WithinRel(0.130694634214992, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 3)), WithinRel(0.999999999999974, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 5)), WithinRel(0.130694634214992, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 6)), WithinRel(0.219074952276694, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 7)), WithinRel(0.0179844363414932, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 0)), WithinRel(0.0179844363415668, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 1)), WithinRel(0.219074952276781, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 2)), WithinRel(0.130694634215074, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 3)), WithinRel(0.999999999999841, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 4)), WithinRel(0.999999999999841, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 5)), WithinRel(0.130694634215074, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 6)), WithinRel(0.219074952276729, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 7)), WithinRel(0.0179844363415669, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 0)), WithinRel(0.0179844363414709, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 1)), WithinRel(0.21907495227672, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 2)), WithinRel(0.130694634215029, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 3)), WithinRel(0.999999999999804, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 4)), WithinRel(0.999999999999804, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 5)), WithinRel(0.130694634215029, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 6)), WithinRel(0.219074952276756, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 7)), WithinRel(0.017984436341471, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 0)), WithinRel(0.0179844363415625, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 1)), WithinRel(0.21907495227653, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 2)), WithinRel(0.130694634215113, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 3)), WithinRel(0.999999999999958, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 4)), WithinRel(0.999999999999958, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 5)), WithinRel(0.130694634215112, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 6)), WithinRel(0.21907495227653, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 7)), WithinRel(0.0179844363415626, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 0)), WithinRel(0.0179844363415261, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 1)), WithinRel(0.21907495227676, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 2)), WithinRel(0.130694634214872, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 3)), WithinRel(0.999999999999829, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 4)), WithinRel(0.999999999999829, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 5)), WithinRel(0.130694634214872, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 6)), WithinRel(0.21907495227676, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 7)), WithinRel(0.0179844363415262, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 0)), WithinRel(0.017984436341519, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 1)), WithinRel(0.219074952276687, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 2)), WithinRel(0.130694634214944, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 3)), WithinRel(0.999999999999826, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 4)), WithinRel(0.999999999999837, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 5)), WithinRel(0.130694634214944, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 6)), WithinRel(0.21907495227674, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 7)), WithinRel(0.0179844363415191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 0)), WithinRel(0.0179844363415044, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 1)), WithinRel(0.219074952276568, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 2)), WithinRel(0.130694634215046, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 3)), WithinRel(0.99999999999972, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 4)), WithinRel(0.99999999999972, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 5)), WithinRel(0.130694634215046, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 6)), WithinRel(0.219074952276612, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 7)), WithinRel(0.0179844363415045, EPSILON_ABS));
            ...
            // Verify page 2
            {
                ...
                // Next, find the maximum of the physical values and normalize for better comparison
                double const abs_max = normalize(values);
                CHECK_THAT(abs_max, WithinRel(0.0038297001770499, EPSILON_ABS));

                // Verify physical values along the surface
                CHECK_THAT(std::abs(values(0, 0)), WithinRel(0.18273276267708, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 1)), WithinRel(0.0933965070365704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 2)), WithinRel(0.429713741670144, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 3)), WithinRel(0.999999999999868, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 4)), WithinRel(0.999999999999868, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 5)), WithinRel(0.429713741670144, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 6)), WithinRel(0.0933965070366334, EPSILON_ABS));
                CHECK_THAT(std::abs(values(0, 7)), WithinRel(0.18273276267708, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 0)), WithinRel(0.182732762677051, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 1)), WithinRel(0.0933965070366252, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 2)), WithinRel(0.42971374167009, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 3)), WithinRel(0.999999999999903, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 4)), WithinRel(0.999999999999907, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 5)), WithinRel(0.42971374167009, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 6)), WithinRel(0.0933965070366272, EPSILON_ABS));
                CHECK_THAT(std::abs(values(1, 7)), WithinRel(0.182732762677051, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 0)), WithinRel(0.182732762677067, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 1)), WithinRel(0.0933965070366563, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 2)), WithinRel(0.429713741670057, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 3)), WithinRel(0.999999999999891, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 4)), WithinRel(0.999999999999891, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 5)), WithinRel(0.429713741670057, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 6)), WithinRel(0.0933965070366563, EPSILON_ABS));
                CHECK_THAT(std::abs(values(2, 7)), WithinRel(0.182732762677067, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 0)), WithinRel(0.182732762676932, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 1)), WithinRel(0.0933965070365914, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 2)), WithinRel(0.429713741670191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 3)), WithinRel(0.999999999999951, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 4)), WithinRel(0.999999999999951, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 5)), WithinRel(0.429713741670191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 6)), WithinRel(0.0933965070365914, EPSILON_ABS));
                CHECK_THAT(std::abs(values(3, 7)), WithinRel(0.182732762676932, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 0)), WithinRel(0.182732762677024, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 1)), WithinRel(0.0933965070366629, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 2)), WithinRel(0.429713741670126, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 3)), WithinRel(0.999999999999872, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 4)), WithinRel(0.999999999999872, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 5)), WithinRel(0.429713741670126, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 6)), WithinRel(0.093396507036612, EPSILON_ABS));
                CHECK_THAT(std::abs(values(4, 7)), WithinRel(0.182732762677024, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 0)), WithinRel(0.182732762677045, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 1)), WithinRel(0.0933965070366875, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 2)), WithinRel(0.429713741670141, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 3)), WithinRel(0.999999999999911, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 4)), WithinRel(0.999999999999911, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 5)), WithinRel(0.429713741670141, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 6)), WithinRel(0.0933965070366601, EPSILON_ABS));
                CHECK_THAT(std::abs(values(5, 7)), WithinRel(0.182732762677045, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 0)), WithinRel(0.18273276267704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 1)), WithinRel(0.0933965070367514, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 2)), WithinRel(0.429713741670093, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 3)), WithinRel(0.999999999999972, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 5)), WithinRel(0.429713741670093, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 6)), WithinRel(0.0933965070367087, EPSILON_ABS));
                CHECK_THAT(std::abs(values(6, 7)), WithinRel(0.18273276267704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 0)), WithinRel(0.182732762677094, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 1)), WithinRel(0.0933965070367039, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 2)), WithinRel(0.429713741670054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 3)), WithinRel(0.999999999999931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 4)), WithinRel(0.999999999999931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 5)), WithinRel(0.429713741670054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 6)), WithinRel(0.0933965070366516, EPSILON_ABS));
                CHECK_THAT(std::abs(values(7, 7)), WithinRel(0.182732762677094, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 0)), WithinRel(0.182732762677094, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 1)), WithinRel(0.0933965070367039, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 2)), WithinRel(0.429713741670054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 3)), WithinRel(0.999999999999931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 4)), WithinRel(0.999999999999931, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 5)), WithinRel(0.429713741670054, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 6)), WithinRel(0.0933965070366516, EPSILON_ABS));
                CHECK_THAT(std::abs(values(8, 7)), WithinRel(0.182732762677094, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 0)), WithinRel(0.18273276267704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 1)), WithinRel(0.0933965070367514, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 2)), WithinRel(0.429713741670093, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 3)), WithinRel(0.999999999999972, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 4)), WithinRel(1, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 5)), WithinRel(0.429713741670093, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 6)), WithinRel(0.0933965070367087, EPSILON_ABS));
                CHECK_THAT(std::abs(values(9, 7)), WithinRel(0.18273276267704, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 0)), WithinRel(0.182732762677045, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 1)), WithinRel(0.0933965070366875, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 2)), WithinRel(0.429713741670141, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 3)), WithinRel(0.999999999999911, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 4)), WithinRel(0.999999999999911, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 5)), WithinRel(0.429713741670141, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 6)), WithinRel(0.0933965070366601, EPSILON_ABS));
                CHECK_THAT(std::abs(values(10, 7)), WithinRel(0.182732762677045, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 0)), WithinRel(0.182732762677025, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 1)), WithinRel(0.0933965070366845, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 2)), WithinRel(0.429713741670111, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 3)), WithinRel(0.999999999999891, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 4)), WithinRel(0.999999999999891, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 5)), WithinRel(0.429713741670111, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 6)), WithinRel(0.0933965070367077, EPSILON_ABS));
                CHECK_THAT(std::abs(values(11, 7)), WithinRel(0.182732762677025, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 0)), WithinRel(0.182732762676932, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 1)), WithinRel(0.0933965070365915, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 2)), WithinRel(0.429713741670191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 3)), WithinRel(0.999999999999951, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 4)), WithinRel(0.999999999999951, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 5)), WithinRel(0.429713741670191, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 6)), WithinRel(0.0933965070365914, EPSILON_ABS));
                CHECK_THAT(std::abs(values(12, 7)), WithinRel(0.182732762676932, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 0)), WithinRel(0.182732762677067, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 1)), WithinRel(0.0933965070366563, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 2)), WithinRel(0.429713741670057, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 3)), WithinRel(0.999999999999891, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 4)), WithinRel(0.999999999999891, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 5)), WithinRel(0.429713741670057, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 6)), WithinRel(0.0933965070366563, EPSILON_ABS));
                CHECK_THAT(std::abs(values(13, 7)), WithinRel(0.182732762677067, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 0)), WithinRel(0.182732762677051, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 1)), WithinRel(0.0933965070366252, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 2)), WithinRel(0.42971374167009, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 3)), WithinRel(0.999999999999903, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 4)), WithinRel(0.999999999999907, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 5)), WithinRel(0.42971374167009, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 6)), WithinRel(0.0933965070366271, EPSILON_ABS));
                CHECK_THAT(std::abs(values(14, 7)), WithinRel(0.182732762677051, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 0)), WithinRel(0.18273276267708, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 1)), WithinRel(0.0933965070365703, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 2)), WithinRel(0.429713741670144, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 3)), WithinRel(0.999999999999868, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 4)), WithinRel(0.999999999999868, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 5)), WithinRel(0.429713741670144, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 6)), WithinRel(0.0933965070366334, EPSILON_ABS));
                CHECK_THAT(std::abs(values(15, 7)), WithinRel(0.18273276267708, EPSILON_ABS));
