...
Bitmap::Bitmap(std::uint16_t width, std::uint16_t height) : width_(width), height_(height), data{static_cast<std::size_t>(width) * static_cast<std::size_t>(height), {0, 0, 0}}
{
    for (std::uint16_t y = 0; y < height_; ++y)
    {
        for (std::uint16_t x = 0; x < width_; ++x) { (*this)[x, y] = {128, static_cast<uint8_t>((y * 255) / height_), static_cast<uint8_t>((x * 255) / width_)}; }
    }
}

Bitmap::bgr_t& Bitmap::operator[](std::uint16_t const x, std::uint16_t const y) { return data[y * width_ + x]; }

const Bitmap::bgr_t& Bitmap::operator[](std::uint16_t const x, std::uint16_t const y) const { return data[y * width_ + x]; }
