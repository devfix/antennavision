...
double constexpr MS_ELV_HERTZIAN = 2.0 / 3.0 * math::square(components::Radiator::HERTZIAN_DIPOLE_LENGTH); /// ms-elv of single Hertzian dipole
...
TEST_CASE("HertzianDipole", "[Radiator][Directivity]")
{
    ...
    components::Antenna ant = components::Radiator::HertzianDipole::create("HertzianDipole", "");
    components::antenna::get_origin(ant) = &reference;
    std::vector<Complex> const coeffs(components::antenna::size(ant), 1.0);
    ...
    transform(thetas,
        ...
        [&](double theta_) { return components::antenna::calc_directivity_from_spherical(ant, theta_, 0.0, WAVELENGTH, coeffs, sim_params); });
    transform(thetas,
        directivities_actual_phi1.begin(),
        [&](double theta_) { return components::antenna::calc_directivity_from_spherical(ant, theta_, 1.0, WAVELENGTH, coeffs, sim_params); });
...
TEST_CASE("HalfWaveDipole direct", "[Radiator][Directivity]")
{
    auto sim_params = setup::SimParams{.system_wavelength = 0.1, .n_polar = 101, .n_azimuth = 201};
    reference::Reference reference;
    components::Antenna ant = components::Radiator::StandingWaveDipole::create("HalfWaveDipole", "", 0.5 * sim_params.system_wavelength);
    components::antenna::get_origin(ant) = &reference;
    std::vector<Complex> const coeffs(components::antenna::size(ant), 1.0);

    auto const actual = components::antenna::calc_directivity_from_spherical(ant, 0.5 * pi, 0, sim_params.system_wavelength, coeffs, sim_params);
    ...
    ojson const js = ojson::parse(R"JSON(
{
  ...
  "antennas": [
    ...
    std::vector<Complex> const coeffs(components::antenna::size(ant), 1.0);
    auto const actual = components::antenna::calc_directivity_from_spherical(ant, 0.5 * pi, 0, su.sim_params.system_wavelength, coeffs, su.sim_params);
    CHECK_THAT(actual, WithinRel(1.640922388, 1e-3));
}
...
TEST_CASE("FullWaveDipole direct", "[Radiator][Directivity]")
{
    auto sim_params = setup::SimParams{.system_wavelength = 0.1, .n_polar = 101, .n_azimuth = 201};
    reference::Reference reference;
    components::Antenna ant = components::Radiator::StandingWaveDipole::create("FullWaveDipole", "", 1.0 * sim_params.system_wavelength);
    components::antenna::get_origin(ant) = &reference;
    std::vector<Complex> const coeffs(components::antenna::size(ant), 1.0);

    auto const actual = components::antenna::calc_directivity_from_spherical(ant, 0.5 * pi, 0, WAVELENGTH, coeffs, sim_params);
    ...
    ojson const js = ojson::parse(R"JSON(
{
  ...
  "antennas": [
    ...
    auto& ant = su.get_antenna("DUT");
    std::vector<Complex> const coeffs(components::antenna::size(ant), 1.0);
    auto const actual = components::antenna::calc_directivity_from_spherical(ant, 0.5 * pi, 0, WAVELENGTH, coeffs, su.sim_params);
    CHECK_THAT(actual, WithinRel(2.4116035252, 1e-3));
}
...
TEST_CASE("3/2-WaveDipole direct", "[Radiator][Directivity]")
{
    auto sim_params = setup::SimParams{.system_wavelength = 0.1, .n_polar = 101, .n_azimuth = 201};
    reference::Reference reference;
    components::Antenna ant = components::Radiator::StandingWaveDipole::create("3/2-WaveDipole", "", 1.5 * sim_params.system_wavelength);
    components::antenna::get_origin(ant) = &reference;
    std::vector<Complex> const coeffs(components::antenna::size(ant), 1.0);
    ...
    ojson const js = ojson::parse(R"JSON(
{
  ...
  "antennas": [
    ...
    components::Antenna const& ula = su.get_antenna("DUT");

    std::vector<Complex> const coeffs(components::antenna::size(ula), 1.0);
    ...
    double const directivity_actual = components::antenna::calc_directivity_from_spherical(ula, target_polar, target_azimuth, wavelength, coeffs, sim_params);

    // Theoretical expectation: D = N for lambda/2 spaced isotropic elements
    double const directivity_expected = static_cast<double>(components::antenna::size(ula));
...
TEST_CASE("Directivity: 8-Element ULA with Z-Directed Dipoles", "[directivity][ula][dipole]")
{
    ...
    auto const& sim_params = su.sim_params;
    double const wavelength = sim_params.system_wavelength;
    components::Antenna const& ula = su.get_antenna("DUT_DIPOLE");

    std::vector<Complex> const weights(components::antenna::size(ula), 1.0);

    double const d_max_expected = math::square(components::antenna::size(ula) * components::Radiator::HERTZIAN_DIPOLE_LENGTH) / calc_analytical_ula_ms_elv(components::antenna::size(ula), 0.5, D0_HERTZIAN, MS_ELV_HERTZIAN);
     ...
     SECTION("Broadside (Y-Axis) - Maximum Directivity")
    {
        ...
        double const d_actual = components::antenna::calc_directivity_from_spherical(ula, target_polar, target_azimuth, wavelength, weights, sim_params);
    ...
    SECTION("Off-Axis (Pattern Multiplication Check)")
    {
        ...
        double const af_mag = std::abs(std::sin(components::antenna::size(ula) * psi / 2.0) / std::sin(psi / 2.0));

        // Normalized AF Power
        double const norm_af_power = math::square(af_mag / static_cast<double>(components::antenna::size(ula)));
