...
namespace factory
{
    namespace
    {
        bool assert_valid_id(std::string_view id)
        {
            for (char c : id)
            {
                ...
                {
                    throw std::runtime_error(std::format("Invalid id '{}', bad character: '{}'", id, c));
                }
            }
            return true;
        }
    ...
    Reference make_reference(json const &reference_desc, std::list<Reference> const &references)
    {
        ...
        auto const translation = get_vec3(reference_desc, "translation", true);
        auto const rotation = get_quaternion(reference_desc, "rotation", true);
    ...
    std::unique_ptr<Radiator> make_radiator(json const &radiator_desc, std::list<Reference> const &references)
    {
        ...
        assert_valid_id(id);
        auto const origin_id = get_string(radiator_desc, "ref", true);
        auto const type = get_string(radiator_desc, "type");
        std::println("Creating radiator [id: '{}', origin: '{}', type: '{}']", id, origin_id, type);
        Reference const &origin = find_reference_by_id(references, origin_id);
        if (type == "HertzianDipole") { return std::make_unique<HertzianDipole>(id, origin, get_double(radiator_desc, "length")); }
